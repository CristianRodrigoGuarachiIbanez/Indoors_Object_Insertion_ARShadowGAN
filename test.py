...
def read_image(image_path:str, channels:int) -> ndarray:
    img:ImageEditor = ImageEditor(image_path)
    image:ndarray;
    SIZE:Tuple[int,int]= (256,256)
    if(channels == 3):
        image = img.reshapeImage(SIZE);
        #image = cv.imread(image_path, cv.IMREAD_COLOR);
        #print(image.shape)
    else:
        #image = cv.imread(image_path, cv.IMREAD_GRAYSCALE)
        image = img.editImgArray(equa_method='gray', scale=SIZE)
        #image = img.addNewChannel(image, 1)
        image = np.expand_dims(image, axis=2)
    image = image.astype(np.float) / 127.5 - 1.0
    print('last image edition',image.shape)
    return image
